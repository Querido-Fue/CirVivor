...
export class UIElementFactory {
    static _handlers = {
        'button': (item, x, y, parentW, parentH, forcedW, layoutHandler) => {
            ...
            let fontFam = props.font || presetData.FONT?.FAMILY || 'arial';
            let fontWeig = props.fontWeight || presetData.FONT?.WEIGHT || '';
            let fontSiz = props.size || (presetData.FONT ? layoutHandler._parseUnit(presetData.FONT.SIZE?.BASE || 'WW', presetData.FONT.SIZE?.VALUE || 1, parentW) : 12);

            let align = props.align || presetData.ALIGN || 'center';
            ...
            // 이전 형식 호환성 유지 (text, iconType -> left / center 배열 변환)
            ...
            if (hasIcon) {
                ...
                props.left.push(icon); // 아이콘은 기본적으로 왼쪽에 배치
            }
            ...
            if (hasText) {
                ...
                textElem.init({
                    ...
                    size: fontSiz,
                    color: props.color,
                    align: hasIcon ? 'right' : align // 아이콘이 있으면 텍스트 오른쪽 정렬
                });

                if (hasIcon) {
                    props.right.push(textElem); // 아이콘이 있으면 오른쪽 정렬로 텍스트 분리
                } else {
        ...
        'progress_bar': (item, x, y, parentW, parentH, forcedW, layoutHandler) => {
            const width = forcedW !== undefined ? forcedW : (item.widthObj ? layoutHandler._parseUnit(item.widthObj.unit, item.widthObj.value, parentW) : layoutHandler._parseUnit('WW', 10, parentW));
