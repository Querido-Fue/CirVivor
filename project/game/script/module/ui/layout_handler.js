...
import { UIElementFactory } from 'ui/ui_element_factory.js';
...
export class LayoutHandler {
    ...
    _parseUnit(unit, value, refSize) {
        ...
        if (unit === 'WW' || unit === 'WH') {
            ...
            // OW (Overlay Width) 등은 이미 parent 단계에서 uiScale 연산이 반영된 영역 안에서 분할하는 것이므로 추가로 곱하지 않음
    ...
    build() {
        ...
        for (const item of this._items) {
            ...
            let finalH = 0;
            if (isBottom) {
                // To get actual height before placing, we run a dry-run finalize if it's an hbox to measure H.
                // Wait, _resolveLayout returns `h` correctly if isHboxChild is false!
                finalH = res.h;
    ...
    _resolveLayout(item, parentW, parentH, isHboxChild) {
        ...
        if (item.widthObj && !isAutoW) {
            ...
            } else actualW = 0; // Only applied to basic items
        }
