...
export class MagneticShieldEffectPass {
    /**
     * 실드 shader program, fullscreen buffer와 impact/dent 업로드 버퍼를 준비합니다.
     ...
     * 유효한 양수 `radius`가 없거나 계산된 scissor 영역이 화면과 만나지 않으면 아무 작업도 하지 않습니다.
     * program, array buffer, vertex attribute, uniform과 scissor 상태를 설정해 fullscreen quad를 그리고 scissor를 비활성화합니다.
     * @param {{radius:number, x?:number, y?:number, fieldRadius?:number, time?:number, alpha?:number, ringThickness?:number, glowWidth?:number, shadowColor?:number[], lowColor?:number[], highColor?:number[], highlightColor?:number[], impacts?:Array<{angle?:number, intensity?:number, width?:number, progress?:number}>, dents?:Array<{angle?:number, depth?:number, width?:number, strength?:number}>}} command - `radius`는 필수이고 나머지는 fallback이 있는 선택 필드입니다.
    ...
    /**
     * 소유한 fullscreen buffer와 실드 shader program을 존재할 때 해제합니다.
