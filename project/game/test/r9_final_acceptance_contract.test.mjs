...
const R9_CPU_OWNER_URLS = Object.freeze([
    new URL(
        '../script/module/ingame/flow/wave_run_coordinator.js',
        import.meta.url
    ),
    new URL(
    ...
    new URL(
        '../script/module/ingame/flow/wave_settlement_coordinator.js',
        import.meta.url
    ),
    new URL(
        '../script/module/ingame/flow/r9_recovery_continuity_contract.js',
        import.meta.url
    )
]);
...
test('R9 actual gate와 Post-R8 production exposure는 exact APPROVED 계약이다', async () => {
    ...
    assert.equal(
        ...
        'node game/test/support/run_nw_r9_wave_settlement.mjs'
    );
    assert.equal(
