...
const source = await readFile(
    new URL('../script/module/display/_theme_transition_controller.js', import.meta.url),
    'utf8'
);
...
const controllerModule = new vm.SourceTextModule(source, {
    context,
    identifier: '_theme_transition_controller.js'
});
...
assert.equal(transitionData.DURATION_SECONDS, 0.4);
assert.equal(transitionData.EASING, 'easeOutExpo');

const draws = [];
const controller = new ThemeTransitionController({
    render(layer, command) {
        draws.push({ layer, command: { ...command } });
    },
    getWidth: () => 1280,
    getHeight: () => 720
});
...
assert.equal(draws.at(-1).command.alpha, 1);

animations[1].complete();
await Promise.resolve();
assert.equal(controller.active, false);
assert.equal(controller.alpha, 0);
