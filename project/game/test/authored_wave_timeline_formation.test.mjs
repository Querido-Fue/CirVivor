...
test('formation vocabulary는 normalize하되 pending/unsupported runtime을 stable code로 거절한다', () => {
    const keepFormationWave = createWave('keep-formation-wave', [{
        timelineEntryId: 'keep',
        type: AUTHORED_WAVE_TIMELINE_COMMAND_TYPE.SPAWN_FORMATION,
        formation: createFormation({ keepFormation: true })
    }]);
    assert.throws(
        () => new WaveDirector({ waveDefinition: keepFormationWave })
    ...
    for (const coordinateSystem of [
        ...
        AUTHORED_FORMATION_COORDINATE_SYSTEM.HEX_OFFSET,
        AUTHORED_FORMATION_COORDINATE_SYSTEM.RING_SLOTS
    ]) {
        const waveDefinition = createWave(`unsupported-${coordinateSystem}`, [{
            timelineEntryId: 'unsupported',
            type: AUTHORED_WAVE_TIMELINE_COMMAND_TYPE.SPAWN_FORMATION,
            formation: createFormation({ coordinateSystem })
        }]);
        assert.throws(
            () => new WaveDirector({ waveDefinition }).init(createFixtureTileMap()),
            (error) => error?.code
                === AUTHORED_WAVE_COMPILE_ERROR_CODE.UNSUPPORTED_COORDINATE_SYSTEM
                && error.coordinateSystem === coordinateSystem
        );
    }
});

test('formation size/layout/symbol/route/walkability는 compile 전에 fail-fast한다', () => {
    const invalidFormations = [
        createFormation({ size: 2 }),
        createFormation({ size: 2, layout: ['C', 'CC'] }),
        createFormation({ size: 1, layout: ['C', '.'] }),
        createFormation({ size: 1, layout: ['X'] }),
...
test('authored compile/spawn source에는 randomness나 formation registry metadata ingress가 없다', async () => {
    const [contractSource, directorSource, adapterSource] = await Promise.all([
        readFile(new URL(
    ...
    assert.doesNotMatch(adapterSource, /formation(?:Group|Member|Timeline)Id/);
    assert.equal(
