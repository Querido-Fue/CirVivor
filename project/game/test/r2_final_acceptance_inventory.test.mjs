...
test('Node/churn/WASM/stress/render-golden/diff automated gates는 versioned command로 고정된다', () => {
    ...
    assert.ok(nodeGate.args.slice(2).every((file) => (
        /^game\/test\/[^/]+\.test\.mjs$/u.test(file)
    )));
    ...
    assert.deepEqual(
        ...
        [
            'game/test/support/run_r2_gpu_churn_soak.mjs --cycles=3',
            'game/test/support/build_enemy_ai_flow_field_wasm.mjs --check',
...
test('final runner는 환경 stage별 실제 capability command를 직렬 실행한다', async () => {
    ...
    assert.match(
        source,
        /process\.execPath,[\s\S]*?\['game\/test\/support\/run_nw_webgpu_capability\.mjs'\]/u
    );
