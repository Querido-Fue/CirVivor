...
// 독립인 source-local query result binding으로 유지합니다.
...
assert.match(
    compute,
    /struct TowerTargetQueryResult \{\s*source_entity_id: u32,\s*source_incarnation: u32,\s*target_slot: u32,\s*target_entity_id: u32,\s*target_incarnation: u32,\s*share_units: u32,\s*group_revision: u32,\s*roster_fingerprint: u32,\s*distance_squared: f32,\s*flags: u32,\s*\}/
);
assert.match(
    compute,
    /fn tower_target_query_is_valid\(source_body_id: u32\)[\s\S]*?TOWER_TARGET_QUERY_FLAG_VALID[\s\S]*?TOWER_TARGET_QUERY_FLAG_SOURCE_VALID[\s\S]*?query\.source_entity_id[\s\S]*?query\.source_incarnation[\s\S]*?query\.target_slot[\s\S]*?query\.target_entity_id[\s\S]*?query\.target_incarnation[\s\S]*?BODY_LAYER_PLAYER_DAMAGEABLE[\s\S]*?GAMEPLAY_TEAM_PLAYER/
);
...
const selectedResolveEnd = compute.indexOf(
    'fn tower_target_query_is_valid(',
