...
function finalHealthAudit(harness) {
    ...
    return Object.freeze({
        ...
        sessionGenerationDelta:
            endpoint.sessionGeneration - harness.initialSessionGeneration,
        recoveryRequired:
...
async function runImpossiblePlacementFixture(device, format) {
    ...
    try {
        ...
        // 유효하지만 destination radius와 14개 고정 offset은 모두 SDF-invalid입니다.
        ...
        assert(cast.outcome.code
            ...
            && reason?.code === 'NO_VALID_PLACEMENT'
            && reason.firstFailingRank === 0
            && reason.attemptedCandidateCount === 14
    ...
    try {
        ...
        for (const slotId of [
            ...
            assert(cast.outcome.code === ABILITY_EXECUTION_OUTCOME_CODE.COMPLETED
                && cast.outcome.generatedCount > 0
                && cast.cooldownConsumed === true,
            `${slotId} actual R2 execution 실패: ${JSON.stringify(cast)}`);
        ...
        assert(harness.stageReceiptEvidence.length >= 4,
            `SHIFT/SPACE stage receipt evidence 부족: ${JSON.stringify(harness.stageReceiptEvidence)}`);

        // 요청 범위 밖인 후속 Rhom priority-control 주기와 이번 입력 회귀를
        // 섞지 않는다. 위 입력들은 실제 R2 wave churn 중 모두 완료됐다.
        ...
        const cleanupTick = harness.fixedTick + 1;
        let cleanedEnemyCount = 0;
        for (const handle of activeEnemyHandles) {
            const receipt = harness.endpoint.requestDespawn(
                handle,
                'post-r5-wave-fixture-settlement',
                cleanupTick,
                `post-r5-wave-cleanup:${handle.entityId}:${handle.incarnation}`
            );
            assert(receipt.accepted === true,
                `wave fixture cleanup 실패: ${JSON.stringify({ handle, receipt })}`);
            cleanedEnemyCount++;
        }
        await advanceFixedTick(harness, cleanupTick);
        assert(registry.getActiveCount('enemy') === 0,
            `wave fixture enemy cleanup 실패: ${registry.getActiveCount('enemy')}`);
...
async function runArrowR2Fixture(device, format) {
    ...
    try {
        ...
        for (let step = 0; step < 180 && !sawRecover; step++) {
            ...
            if (behavior.state === GPU_CIRCLE_ENEMY_BEHAVIOR_STATE.WINDUP) {
                ...
                near(behavior.chargeAccelerationTilesPerSecondSquared,
                    12, 0.000001, 'Arrow authored acceleration');
                near(behavior.chargeSpeedTilesPerSecond,
                    6, 0.000001, 'Arrow authored max speed');
                const speed = (arrow.velocity.x * lockedDirection.x)
            ...
            if (movedTower && towerPositionAfterMove === null) {
                const towerBody = findBody(bodies, towerHandle, 'post-move Tower');
                if (towerPositionBeforeMove
                    && Math.hypot(
                        towerBody.position.x - towerPositionBeforeMove.x,
                        towerBody.position.y - towerPositionBeforeMove.y
                    ) > 0.0001) {
                    towerPositionAfterMove = Object.freeze({ ...towerBody.position });
                }
            }
            previousArrow = arrow;
        }
        ...
        `Arrow actual state sequence 불일치: ${JSON.stringify({ sawWindup, chargeSamples, recoilSamples, sawRecover, lockedDirection, movedTower, stoppedTower, towerPositionBeforeMove, towerPositionAfterMove, contactEvents: [...contactEvents.values()] })}`);
        for (let index = 0; index < chargeSamples.length; index++) {
            const sample = chargeSamples[index];
            assert(sample.speed > 0 && sample.speed <= 6.0001,
                `Arrow charge speed 범위 실패: ${JSON.stringify(sample)}`);
            if (index > 0) {
                assert(sample.speed + 0.0001 >= chargeSamples[index - 1].speed,
                    `Arrow charge speed가 감소했습니다: ${JSON.stringify(chargeSamples)}`);
            }
        }
        near(chargeSamples[0].speed, 12 / 60, 0.025,
            'Arrow first acceleration tick');
        assert(chargeSamples[0].displacement
            < chargeSamples[Math.floor(chargeSamples.length / 2)].displacement,
        `Arrow first displacement가 middle보다 작지 않습니다: ${JSON.stringify(chargeSamples)}`);
        for (let index = 1; index < recoilSamples.length; index++) {
            assert(recoilSamples[index].speed
                <= recoilSamples[index - 1].speed + 0.0001,
            `Arrow recoil Expo-out이 감소하지 않습니다: ${JSON.stringify(recoilSamples)}`);
        }
        const health = finalHealthAudit(harness);
        assert(health.restartCountDelta === 0
            && health.sessionGenerationDelta === 0
        ...
        return Object.freeze({
            ...
            chargeMaxSpeedTilesPerSecond: 6,
