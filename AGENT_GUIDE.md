...
- native 포트는 독립 실행되는 C++20 rewrite입니다. `project/`의 JavaScript/NW.js는 관찰 가능한 화면·입력·상태와 fixture를 얻는 read-only oracle일 뿐이며, native 제품에 JavaScript engine/interpreter를 넣거나 Promise·microtask·부동소수점 같은 ECMAScript 런타임 세부 동작을 에뮬레이션하지 않습니다.
...
- native UI 상태는 `ui_runtime`의 `TitleOverlayStateMachine::advance(deltaSeconds)`가 가변 표시 시간으로 진행하며 60Hz `tick()`은 테스트 adapter일 뿐입니다. 외부 URL 실행은 sequence가 있는 effect를 플랫폼에 넘긴 뒤 success/failure acknowledge를 받아야 하며, 종료 확인은 one-shot latch 하나만 권위로 사용합니다. 레이아웃은 논리 safe-area inset을 먼저 제외하고 light/dark 토큰과 타이틀 entrance/exit/external geometry를 계산합니다. 이 기반만으로 실제 UI 콘텐츠나 pixel parity가 완료된 것은 아닙니다.
- native UI 입력 경계는 `PlatformEvent`의 SDL 비의존 mouse/touch/cancel/wheel/고정 UTF-8·IME payload를 사용합니다. `SDL_EVENT_WINDOW_CLOSE_REQUESTED`는 `windowCloseRequested`로 번역해 향후 exit overlay가 소비할 수 있게 하고, 소비자가 없는 현재는 `Application`이 기존 정상 종료 fallback을 유지합니다.
