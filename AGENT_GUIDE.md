...
- native 설정 기반은 `native/src/settings/`의 SDL 비의존 `settings_runtime`이 소유합니다. 15개 설정 키와 고정 용량 입력 바인딩을 강타입 모델로 유지하고, 64KiB 상한·strict JSON/UTF-8/중복 키·legacy migration/clamp·결정적 canonical encode를 `settings_codec`에서 처리합니다. `SettingsRepository`는 missing/default, legacy·손상 canonical 복구와 `settings.json.tmp` write→replace를 담당하며 `SdlSettingsStorage`가 이를 SDL user storage에 연결합니다. `Application` load/apply와 Settings control effect는 후속입니다.
- native debug 입력·frame 정책은 `native/src/debug/DebugRuntimeController`가 SDL/UI/JS 비의존 고정 용량 상태로 소유합니다. 같은 pointer의 middle press/release 3회를 첫 release부터 2초 이내에 받아 debug mode effect를 만들며 focus/cancel에서 gesture와 key edge를 지우고 focus loss에서는 예약된 one-shot도 취소합니다. `/` pause와 `.` one-shot은 UI/render를 계속 실행한 채 gameplay fixed-step budget만 각각 0/1로 제한합니다. 이 계층은 현재 순수 runtime foundation까지만 완료됐고 `PlatformEvent`, overlay control, `Application`/settings 저장 연결은 후속입니다.
