...
> **기준 코드**: 0.52 native settings schema + title overlay breadth + title→playable slice | **전환 상태**: NW.js read-only oracle + Windows C++20/SDL3 rewrite 병행 | **렌더링**: Canvas 2D + WebGL oracle, SDL_GPU/GLES/Software native 경로 병행
...
- native 설정 기반은 `native/src/settings/`의 SDL 비의존 `settings_runtime`이 소유합니다. 15개 설정 키와 고정 용량 입력 바인딩을 강타입 모델로 유지하고, 64KiB 상한·strict JSON/UTF-8/중복 키·legacy migration/clamp·결정적 canonical encode를 `settings_codec`에서 처리합니다. 이 계층은 현재 schema/codec까지만 완료됐으며 SDL user storage repository, `Application` 적용과 Settings control effect는 후속입니다.
