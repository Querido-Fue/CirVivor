...
> **기준 코드**: 0.48 연속 UI interaction retarget 베이스라인 | **전환 상태**: NW.js read-only oracle + Windows C++20/SDL3 rewrite 병행 | **렌더링**: Canvas 2D + WebGL oracle, SDL_GPU/GLES/Software native 경로 병행
...
- native 제품 실행은 `Application`이 기본적으로 순수 C++ title runtime과 `title_scene`을 빌드합니다. 최소 `GameSystem`/`playable_game_scene`은 `--playable-scene`, synthetic scene은 smoke와 `--diagnostic-scene` 전용입니다. 타이틀 메뉴에서 playable session으로 넘어가는 제품 흐름은 아직 미구현입니다. `app_runtime`의 `MovementInputBuffer`는 물리 source alias를 합성하고 짧은 press를 첫 fixed tick까지 보존하며 focus/background 전환에서 held/pending을 함께 지웁니다.
...
- native UI 상태는 `ui_runtime`의 `TitleOverlayStateMachine::advance(deltaSeconds)`가 가변 표시 시간으로 진행하며 60Hz `tick()`은 테스트 adapter일 뿐입니다. `Application`은 이를 title display frame마다 한 번 진행하고 `title_scene` presenter로 `FramePacket`을 만듭니다. 외부 URL 실행은 sequence가 있는 effect를 플랫폼에 넘긴 뒤 success/failure acknowledge를 받아야 하며, 직접 버전 링크는 sequence 0 handoff입니다. 종료 확인은 one-shot latch 하나만 권위로 사용합니다. 레이아웃은 논리 safe-area inset을 먼저 제외하고 light/dark 토큰과 타이틀 entrance/exit/external geometry를 계산합니다. 8개 메뉴 overlay 본문·실제 text/texture·title→playable 전환은 아직 미구현입니다.
