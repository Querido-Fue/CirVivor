...
> **기준 코드**: 0.45 해상도 독립 타일 월드 베이스라인 | **런타임**: NW.js 데스크톱 | **렌더링**: Canvas 2D + WebGL
...
- 신규 인게임은 `1 실제 타일 = 1 월드 단위`, `Tower 지름 = 1타일` 계약을 사용합니다. `WorldCamera2D`가 `min(viewportWidth / worldWidth, viewportHeight / worldHeight)` contain 배율을 resize 때 계산하며 물리·AI·저장 좌표는 resize로 바꾸지 않습니다.
- 정적 타일의 viewport 좌표는 최초 draw와 projection revision 변경 때만 캐시를 다시 만들고, 동적 entity만 프레임마다 투영합니다. 대량 entity 단계에서는 동일 계약을 GPU view-projection uniform으로 옮길 수 있습니다.
