...
> **기준 코드**: 0.59 native input binding runtime + Debug telemetry HUD + settings/debug integration | **전환 상태**: NW.js read-only oracle + Windows C++20/SDL3 rewrite 병행 | **렌더링**: Canvas 2D + WebGL oracle, SDL_GPU/GLES/Software native 경로 병행
...
- native 제품 실행은 `Application`이 기본적으로 순수 C++ title runtime과 `title_scene`을 빌드합니다. Start 카드의 MapSelect에서 단일 권위 map ID를 확인한 뒤 후보 `GameSystem`을 먼저 만들고, 성공할 때만 title 상태를 정리해 `playable_game_scene`으로 전환합니다. 실패하면 MapSelect를 유지하고 입력을 다시 열어 재시도할 수 있습니다. `--playable-scene`은 개발용 직접 진입, synthetic scene은 smoke와 `--diagnostic-scene` 전용입니다. SDL 경계는 scancode를 DOM-style 물리 code로만 번역하고, SDL 비의존 `input_runtime/InputBindingMap`이 JS와 같은 9개 기본 action과 최대 4개 사용자 override를 의미 action으로 컴파일합니다. 같은 action의 alias는 첫 press/마지막 release에서만 전환되고 같은 code의 복수 action은 모두 전달됩니다. `Application`은 movement/debug action을 실제 소비자에 연결하며 설정 교체·focus/background·세션 전환에서 mapper와 held/pending 상태를 함께 지웁니다. `MovementInputBuffer`는 짧은 press를 첫 fixed tick까지 보존합니다.
...
- native 설정 기반은 `native/src/settings/`의 SDL 비의존 `settings_runtime`이 소유합니다. 15개 설정 키와 고정 용량 입력 바인딩을 강타입 모델로 유지하고, 64KiB 상한·strict JSON/UTF-8/중복 키·legacy migration/clamp·결정적 canonical encode를 `settings_codec`에서 처리합니다. `SettingsRepository`는 missing/default, legacy·손상 canonical 복구와 `settings.json.tmp` write→replace를 담당하며 `SdlSettingsStorage`가 이를 SDL user storage에 연결합니다. `SettingsOverlaySession`은 overlay sequence별 baseline/draft와 10개 노출 필드의 dirty mask를 소유하고 Save 후보에는 repository authority의 숨김 설정/input binding을 보존합니다. `Application`은 storage-ready 뒤 exactly-once load하고 overlay sequence별 live preview/Save/Cancel을 조립합니다. window mode는 같은 SDL window handle에 적용하고 Cancel에서 baseline으로 되돌리며, theme/language/UI·render scale/widescreen/불투명 UI와 input binding은 native consumer에 연결됩니다. tooltip/BGM/SFX와 benchmark는 값 보존·표시와 별개로 실제 consumer가 아직 남아 있습니다.
- native debug 입력·frame 정책은 `native/src/debug/DebugRuntimeController`가 SDL/UI/JS 비의존 고정 용량 상태로 소유합니다. 같은 pointer의 middle press/release 3회를 첫 release부터 2초 이내에 받아 debug mode effect를 만들며 focus/cancel에서 gesture와 key edge를 지우고 focus loss에서는 예약된 one-shot도 취소합니다. SDL event seam은 millisecond timestamp와 middle pointer identity, raw keyboard code/press/repeat를 운반하고 `InputBindingMap`이 `debugPause`·`debugStep` action을 해석합니다. `Application`은 이 effect를 모든 scene의 공용 Debug overlay와 settings repository에 연결하고 저장 실패를 유한 지연 재시도로 추적합니다. `FrameScheduler`의 scheduled/disabled/single-step은 UI·render를 유지하면서 gameplay debt를 지우고 one-shot을 정확히 한 fixed tick으로 제한합니다. `render/frontend/global_debug_overlay`는 title Debug와 같은 dim/session/layer/rounded-clip/content 계약을 playable·diagnostic의 기존 `FramePacketBuilder`에 정확한 additive capacity로 합성하며 실패 시 scene 전체 packet을 abort합니다. dim은 builder의 active viewport와 실제 renderer scale로 full drawable을 덮되 panel은 중앙 logical content에 유지합니다. 별도 `DebugPerformanceTracker`는 기존 scheduler용 `previousFrameCpuSeconds_`의 의미를 바꾸지 않고 성공한 display frame의 전체 wall/update+build/fixed/scene-build/render-call 구간을 섹션별 512-sample·최근 1초 고정 링에 보관합니다. `debug_telemetry_hud`는 panel open 여부와 독립적으로 title/playable/diagnostic에 좌상단 avg/last/max, 좌하단 실제 `BodySoA`·직전 `FramePacket`·glyph-atlas 사용량, playable의 보간된 Tower collision circle을 정확한 additive capacity로 기록합니다. 현재 native에 아직 없는 적 전용 dual-radius hitbox는 적 gameplay 이식 때 실제 enemy collision authority에서 추가합니다.
- native UI 상태는 `ui_runtime`의 `TitleOverlayStateMachine::advance(deltaSeconds)`가 가변 표시 시간으로 진행하며 60Hz `tick()`은 테스트 adapter일 뿐입니다. `Application`은 이를 모든 display frame에서 pause/single-step 정책과 함께 진행하고 Settings session 동기화만 title에 한정합니다. title은 `title_scene`, playable·diagnostic의 Debug 한 장은 공용 global composer로 같은 presentation을 `FramePacket`에 기록합니다. 버전과 credits를 포함한 외부 URL은 warning overlay의 sequence가 있는 effect를 플랫폼에 넘긴 뒤 success/failure acknowledge를 받아야 합니다. Credits의 다섯 stable control ID는 고정 HTTPS URL로만 resolve하고 같은 overlay sequence·control ID의 pointer down/up에서 warning을 엽니다. 종료 확인은 one-shot latch 하나만 권위로 사용합니다. MapSelect의 playable effect도 overlay sequence와 강타입 map ID를 사용하며 실패 acknowledge에서 interaction lock을 해제합니다. 레이아웃은 논리 safe-area inset을 먼저 제외하고 light/dark 토큰과 타이틀 entrance/overlay geometry를 계산합니다. `TitleOverlayPresentationSet`은 상태·레이아웃·application-control revision에 결합 가능한 fixed-capacity DTO이며 renderer와 hit-test가 stable control ID, enabled/selected/value와 오른쪽 `valueRect`를 공유합니다. Settings 10개 control과 Debug 네 toggle의 application-control effect는 제품 `Application`이 소비합니다. `app/title_display_policy`는 widescreen OFF 및 세로형 화면을 16:9 content-local 좌표계로 contain하고 레이아웃·safe-area·pointer 원점을 함께 바꾸며, ON인 16:9 이상 화면만 전체 폭을 사용합니다. playable은 같은 설정의 world rect 밖을 backend 공통 opaque UI mask 최대 두 개로 차폐하고 HUD용 중앙 16:9 좌표계를 유지합니다. 불투명 UI는 glass pass를 생략하고 opaque panel token을 사용합니다. logo·utility icon·GPU glyph/effect와 세부 시각 fidelity는 아직 남아 있습니다.
- native UI 입력 경계는 `PlatformEvent`의 SDL 비의존 mouse/touch/cancel/wheel/고정 UTF-8·IME payload를 사용합니다. `app/logical_ui_projection`은 window→drawable content rect→logical content rect 좌표를 backend viewport가 실제 사용하는 `drawablePixelsPerLogicalUnitX/Y`로 clamp 없이 transactional하게 역변환하고 `Application`이 이를 non-title Debug hit-test에 사용합니다. Debug attachment가 존재하는 동안 pointer를 modal로 먼저 소비하며, attachment가 없으면 향후 gameplay pointer 경로로 fall-through합니다. 0×0/minimized drawable의 transient pointer는 capture만 지우고 application failure로 승격하지 않습니다. `SDL_EVENT_WINDOW_CLOSE_REQUESTED`는 `windowCloseRequested`로 번역하며 title 모드에서는 exit-confirm overlay가 소비합니다. 개발용 playable/diagnostic 모드만 기존 즉시 종료 fallback을 유지합니다.
- native renderer는 한 SDL window에 여러 그래픽 API 자원을 동시에 붙이지 않습니다. `Application`의 Router factory가 후보마다 숨김 창을 재생성하며, `neutral → SDL_GPU`, `SDL_WINDOW_OPENGL + 사전 GL attributes → GLES ES3/ES2`, `neutral → Software` 순서로 하나의 backend만 소유하게 합니다. 모든 scene에서 공용 UI glyph resource를 제출하므로 현재는 glyph-atlas capability gate가 SDL_GPU/GLES를 건너뛰고 Software를 선택합니다. GPU/GLES atlas 구현 뒤 같은 gate를 통과시키며, 종료는 backend를 먼저 내린 뒤 window를 파괴합니다. backend 중립 `lastFrameContentHash()`는 Debug/통합 진단용 관찰값일 뿐 렌더 정책의 권위가 아니며 Software는 present 전 CPU raster hash를 제공합니다.
- 런타임 fullscreen/windowed 변경은 renderer가 참조하는 `SDL_Window`를 파괴하지 않고 `SdlWindow::applyDisplayConfiguration()`으로 같은 handle에 적용합니다. 실패하면 직전 실제 mode/windowed size로 best-effort rollback하며, backend fallback의 `recreate()`에는 마지막 성공 display configuration을 보존하고 최종 `shutdown()`에서만 기본 windowed 1280×720로 초기화합니다.
