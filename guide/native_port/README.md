...
| `native/src/input/` / `input_runtime` | DOM-style 물리 code를 9개 의미 action으로 컴파일하고 alias의 첫 press/마지막 release를 집계하는 SDL 비의존 고정 용량 입력 맵 |
...
| `native/src/debug/` / `debug_runtime` | Debug gesture·toggle·pause/step 정책과 renderer/SDL 비의존 최근 1초 성능 구간 tracker |
...
SDL keyboard 경계는 scancode를 `KeyW`, `ArrowUp`, `Slash` 같은 DOM-style 물리 code로만 변환한다. `input_runtime/InputBindingMap`은 JS와 같은 9개 기본 action과 action별 최대 4개 `settings.json` override를 고정 배열에 컴파일하고, 명시적인 빈 override는 해당 action을 unbind한다. 같은 action의 여러 alias는 첫 press와 마지막 release에서만 aggregate 전환을 만들며 같은 물리 code에 연결된 여러 action은 모두 전달한다. repeat·중복 press·대응 없는 release는 새 전환을 만들지 않는다. `Application`은 movement action을 `MovementInputBuffer`, debug pause/step을 `DebugRuntimeController`에 연결하고, 아직 gameplay 소비자가 없는 primary/pause/reload도 mapper 계약에는 보존한다. 설정 교체·focus/background·scene 전환·shutdown에서는 held mapper와 movement/debug key edge를 함께 지워 phantom input을 막는다. `MovementInputBuffer`는 같은 SDL event batch에서 keydown과 keyup이 모두 도착해도 press를 첫 fixed step까지 latch하고, held action은 모든 fixed step에 유지한다.
...
`Application`은 이 runtime을 기본 실행 경로에서 소유하고 title `FramePacket`을 만든다. title pointer, 창 닫기, exit/external confirmation과 Start→MapSelect→playable 전환까지 연결했다. 버전 링크도 JS 제품과 같이 External warning을 먼저 연다. Software backend는 원본 Pretendard로 미리 만든 title/card/version 및 11종 overlay 문자열을 실제 A8 glyph로 그리며 MapSelect 5×9 preview, Deck 0% 카드, 네 dummy 화면, Settings의 현재 값, Credits 5개 행, Debug panel과 Exit/External dialog를 표시한다. 공통 Close/Cancel과 Map 시작은 실제 pointer release를 소비한다. Credits의 다섯 stable control ID는 고정 HTTPS URL로 resolve해 같은 control에서 pointer를 놓을 때 External warning을 열고 기존 confirm→`SDL_OpenURL`→sequence acknowledge를 재사용한다. Settings의 10개 control은 overlay draft를 live preview하고 Save/Cancel로 저장/rollback하며, opaque UI는 glass pass 자체를 생략한다. `title_display_policy`는 widescreen OFF와 모든 세로형 화면에서 title·overlay 레이아웃, safe-area, mouse/touch를 하나의 16:9 content-local 좌표계로 contain하고, ON인 16:9 이상 화면만 전체 폭으로 확장한다. title letterbox는 JS body와 같은 `#202020` clear를 사용하고 playable은 world/effect 뒤·HUD 앞의 opaque drawable mask로 확대 카메라의 bar 침범을 세 backend에서 막는다. Debug는 2초 내 middle release 3회로 열고 네 toggle과 바인딩된 pause/step action을 소비한다. frame-time은 최근 1초 avg/last/max, pool은 native 실제 저장소 사용량, hitbox는 현재 playable Tower의 보간된 collision circle을 panel open 상태와 독립적으로 표시한다. 로고·utility icon·texture, floating dropdown, 실제 audio/tooltip consumer, 적 이식 뒤의 적 전용 dual-radius hitbox, GPU 계열 atlas/고급 렌더와 21개 native 시각 회귀도 남아 있으므로 타이틀·오버레이 완료로 판정하지 않는다.

`settings_runtime`은 SDL과 UI에 의존하지 않는 15-key `GameSettings` 모델, 고정 용량 `KeyboardEvent.code` override와 strict canonical JSON codec을 소유한다. decoder는 64KiB·UTF-8·중복 키·중첩/멤버/문자열 상한과 required type을 검사하고, 기존 `darkMode`/`borderless`/제거된 키를 migration하면서 수치 범위를 clamp한 뒤 canonical rewrite 필요 여부를 돌려준다. encoder는 유효한 모델만 고정 key/number 형식으로 출력한다. `SettingsRepository`는 missing 파일의 기본값, legacy/non-canonical 재작성, 손상·64KiB 초과 복구와 `settings.json.tmp` write→replace 저장을 소유하고, `SdlSettingsStorage`가 실제 SDL user storage에 연결한다. 저장 실패에서는 기존 파일과 메모리 값을 보존하고 tmp를 best-effort 정리한다. `Application`은 storage-ready 뒤 한 번만 load하고 sequence별 `SettingsOverlaySession`의 baseline/draft를 theme·language·UI/render scale·window mode·widescreen·opaque render와 input binding에 적용한다. Save는 현재 debug authority와 숨김 키를 보존해 repository에 합치고, Cancel은 closing presentation이 끝난 뒤 같은 window handle과 나머지 runtime을 baseline으로 되돌린다. BGM/SFX·tooltip delay와 benchmark의 실제 consumer는 아직 남아 있다.

`debug_runtime`은 SDL·overlay·저장소와 분리된 `DebugRuntimeController`와 `DebugPerformanceTracker`를 제공한다. 동일 pointer의 middle press/release 3회가 첫 release부터 정확히 2,000ms 이내일 때 debug mode와 overlay open/close·persist intent를 만들고, focus/cancel에서 gesture와 눌린 key edge를 정리하며 focus loss에서는 예약된 one-shot도 취소한다. `Application`은 SDL millisecond timestamp와 pointer identity를 controller에 전달하고, raw keyboard code는 `InputBindingMap`을 거쳐 설정 가능한 debug pause/step action으로 전달한다. 저장 실패는 런타임 debug authority를 유지한 채 최대 3회 지연 재시도하며 다음 명시적 Settings Save도 이를 합친다. frame-time/pool/hitbox/animation 네 표시 toggle은 모두 실제 consumer에 연결되고, paused/single-step frame은 UI·render를 계속 허용하면서 gameplay fixed-step budget만 0/1로 제한한다.

성능 tracker는 기존 `previousFrameCpuSeconds_`를 renderer 포함 값으로 바꾸지 않는다. 그 값은 계속 `FrameScheduler`의 CPU-bound 판단에만 쓰고, 별도 고정 링이 성공적으로 렌더된 frame의 전체 active wall, update+build, fixed total, scene build, render-call wall 구간을 섹션마다 최대 512개·최근 1초로 집계한다. `render-call`은 GPU timestamp가 아니라 backend `render()` 호출의 wall time이다. HUD는 완료된 직전 frame snapshot만 읽고 렌더 실패에서는 갱신하지 않는다.

`render/frontend/debug_telemetry_hud`는 title/playable/diagnostic 장면에 정확한 additive `FramePacketCapacity`로 합성한다. 좌상단 profiler는 `frame.cpu`를 항상 표시하고 0.01ms 이상인 나머지 구간을 평균 내림차순으로 보여 준다. 좌하단 pool은 JS pool을 흉내 내지 않고 `BodySoA enabled/slots/capacity`, 직전 `FramePacket used/capacity`, `GlyphAtlas entries/capacity`만 표시한다. 동적 숫자는 preload한 단일 Pretendard glyph를 고정 배열로 조합해 frame build에서 shaping·heap allocation을 만들지 않는다. hitbox-only 구성은 text resource 없이도 동작한다. 현재 playable은 실제 tile solver에 참여하는 Tower 원을 시각 도형과 같은 previous→current 보간 위치에 표시한다. 기존 JS의 enemy-pair/projectile dual-radius 원은 적 gameplay 이식 뒤 실제 enemy collision authority에서 공급해야 한다.
...
`input_binding_map_tests`는 JS 기본 binding, override/unbind, alias aggregate, 한 code의 복수 action, repeat/중복 edge, invalid 교체의 원자성, 의미상 같은 재적용의 held 보존과 반복 처리 무할당을 검사한다. `movement_input_buffer_tests`는 짧은 down/up pulse, held input, repeat idempotence, 복수 alias source, focus/background에 대응하는 clear 계약을 검사한다. `title_display_policy_tests`는 ultrawide ON/OFF, 세로형·정확한 16:9, 비대칭 safe-area, 레터박스 밖 pointer와 invalid 입력의 원자적 보존을 검사한다. `sdl_platform_event_tests`는 raw DOM-style keyboard code와 repeat/timestamp, mouse/touch/cancel/wheel, UTF-8 경계 절단, IME composition, focus clear와 dismissible window-close seam을 검사한다. `font_stack_tests`는 WOFF2/OFL hash, memory face, 누락 emoji asset 정책, canonical 한국어/라틴 shaping과 shaped cache의 generation·원자적 실패를 검사한다. `software_renderer_tests`는 실제 A8 nearest/linear sampling, PMA, transform, 이중 clip, invalid resource와 반복 render 무할당을 검사한다. `playable_game_scene_tests`는 94-command 기본 장면과 최대 96-command letterbox 장면, 보간·safe area·DPI·capacity transaction, 반복 build의 무할당, widescreen on/off 월드/UI viewport 및 wide/tall opaque mask를 검사한다.

`title_overlay_state_machine_tests`는 가변 frame rate, 큰 delta 제한, Loading→Title carry, overlay open/close retarget, Debug pause/focus, UTF-8 URL 경계·scheme/authority와 플랫폼 acknowledge, exactly-once 종료 및 무할당 snapshot을 검사한다. `ui_layout_metrics_tests`는 16:9/ultrawide, zero-inset Desktop과 비대칭 Android safe-area, uiScale, light/dark token, 타이틀 entrance 중간값, exit/external geometry와 transactional invalid input을 검사한다. `title_ui_controller_tests`는 latest-sequence top input, shared rounded rect, 8종 breadth Close와 application-control revision을 검사한다. `title_scene_tests`는 11종 resource-backed content, partial text table의 transactional rollback, glass/opaque pass·capacity 분기, fixed `#202020` letterbox clear와 full/contained viewport, worst overlay stack의 fixed capacity·반복 build 무할당과 responsive target size/transform을 검사한다.

`settings_codec_tests`는 15-key 기본값·강타입 validation과 원자적 runtime 교체, `userLanguage` 및 input binding canonical round-trip, clamp/legacy migration, strict JSON·UTF-8·duplicate/trailing/type 오류, 64KiB·중첩·멤버 상한과 손상된 public model 값의 범위 안전성을 검사한다.

`settings_repository_tests`는 missing/default, canonical·legacy·손상·oversized load와 tmp write/replace/cleanup 실패의 디스크·메모리 보존을 검사한다. `sdl_settings_storage_tests`는 분리된 테스트 namespace에서 실제 SDL user storage round-trip과 손상 복구를 확인하고 생성 파일을 정리한다.

`debug_runtime_controller_tests`는 2,000ms 경계를 포함한 matched middle-release 3회, focus/cancel reset, debug enable/disable·overlay intent, 네 display toggle, key repeat/duplicate edge와 running/paused/single-step fixed-step budget을 검사한다. `debug_performance_tracker_tests`는 비활성 gate, 정확한 1초 경계, 섹션 독립성, 비단조 timestamp reset, 512-sample overwrite와 disable reset을 검사한다. `debug_telemetry_hud_tests`는 pool/frame/hitbox exact capacity, avg 정렬, 고정 glyph 숫자 조합, text-free hitbox-only 경로, hitbox-first top order, invalid-before-mutation, capacity abort와 반복 build 무할당을 검사한다.

`application_integration_smoke_tests`는 격리된 SDL user-storage namespace에 비기본 설정과 input override를 seed한 뒤 실제 `Application` title/diagnostic/playable Software 경로를 실행한다. 공개 event API로 변경된 이동 키와 Debug gesture·control·변경된 pause/step 키·동적 telemetry를 통과시키며, 하나의 physical code에 연결된 movement/debug action batch 전체 처리와 focus lost/gained 뒤 held edge 초기화도 검증한다. 재시작 전후 `debugMode` 저장과 나머지 설정 보존을 비교한다. pause 상태 hash 검증에서는 실제 control로 frame-time을 끄고 one-frame-delayed FramePacket 사용량을 안정화한 뒤 simulation 정지와 정확한 한 step을 비교하며, 다른 storage smoke와 resource lock을 공유한다.
