...
`application_integration_smoke_tests`는 격리된 SDL user-storage namespace에 비기본 설정을 seed한 뒤 실제 `Application` title/diagnostic/playable Software 경로를 실행한다. 공개 event API로 Debug gesture·control·동적 telemetry·pause/step을 통과시키고 재시작 전후 `debugMode` 저장과 나머지 설정 보존을 검증한다. pause 상태 hash 검증에서는 실제 control로 frame-time을 끄고 one-frame-delayed FramePacket 사용량을 안정화한 뒤 simulation 정지와 정확한 한 step을 비교하며, 다른 storage smoke와 resource lock을 공유한다.
