...
`debug_runtime`은 SDL·overlay·저장소와 분리된 `DebugRuntimeController`를 제공한다. 동일 pointer의 middle press/release 3회가 첫 release부터 정확히 2,000ms 이내일 때 debug mode와 overlay open/close·persist intent를 만들고, focus/cancel에서 gesture와 눌린 key edge를 정리하며 focus loss에서는 예약된 one-shot도 취소한다. `Application`은 SDL millisecond timestamp와 pointer identity, `/`·`.` key edge를 controller에 전달하고 title Debug overlay와 repository 저장을 조립한다. 저장 실패는 런타임 debug authority를 유지한 채 최대 3회 지연 재시도하며 다음 명시적 Settings Save도 이를 합친다. frame-time/pool/hitbox/animation 네 표시 toggle 중 animation은 pause/step에 실제 연결되고, paused/single-step frame은 UI·render를 계속 허용하면서 gameplay fixed-step budget만 0/1로 제한한다. playable/diagnostic 위 global overlay 합성과 frame-time/pool/hitbox 실제 계측 consumer는 아직 남아 있다.
...
`debug_runtime_controller_tests`는 2,000ms 경계를 포함한 matched middle-release 3회, focus/cancel reset, debug enable/disable·overlay intent, 네 display toggle, key repeat/duplicate edge와 running/paused/single-step fixed-step budget을 검사한다.

`application_integration_smoke_tests`는 격리된 SDL user-storage namespace에 비기본 설정을 seed한 뒤 실제 `Application` title/Software 경로를 실행한다. 공개 event API로 Debug gesture·control·pause/step을 통과시키고 재시작 전후 `debugMode` 저장과 나머지 설정 보존을 검증하며, 다른 storage smoke와 resource lock을 공유한다.
