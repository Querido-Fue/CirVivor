...
>
> Safe-boundary schema must support TowerGroup base stats, deterministic share ledger, Lost Share,
> and zero living Towers. The old rule rejecting all Tower health/down keys is superseded. Mid-wave
> body transforms remain excluded.
...
│  ├─ shareScale
│  ├─ lostShareUnits
│  └─ safeBoundaryLivingTowers | aggregate
├─ economy
│  └─ gold
├─ dictionary
│  ├─ activeWordIds
...
Safe-boundary save에는 TowerGroup base stats, Lost Share, and validated living Tower aggregate/records가 존재한다. Mid-wave Tower position/velocity와 transient down/merge travel state는 저장하지 않는다.
...
- CompiledAbility
- Body ABI v8 `AtomicTransformState`, J first-hit PENDING, C′ return due tick, lineage roster,
  prepared/armed transform proof/readback/transaction/command identity
- `ProjectileCaptureState`/candidate planes, held R/projectile roster, capture/release proof/replay/readback,
  active metadata revision/CAS plan, and split immutable-origin/current-allegiance snapshot
- RouteRuntime body state, optional route availability/lease versions, closed paths, Z exact roster,
  route cleanup/readback evidence, and Wave all-closed backlog binding
- VFX/Audio/UI animation
- raw EventJournal
- pause reason
- benchmark/profiler sample

재개 시 다음 웨이브는 깨끗한 월드와 초기화된 cooldown/Effect/Formation/Atomic Transform/
Projectile Capture/Route Runtime 상태에서 시작한다. Optional route graph은 authored content로 다시
컴파일하되 availability는 all-open으로 시작한다. Mid-wave J/C′ lineage나 held/released projectile transaction을
checkpoint에서 복구하지 않는다.
...
- TowerGroup health/share/Lost Share는 새 schema에서 검증한다. legacy reboot key는 명시적 migration 없이 거절한다.
