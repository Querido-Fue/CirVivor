...
→ KeyboardInputHandler raw down set/press edge
→ InputBindingMap의 의미 action
...
- 라우터는 context 역순, priority 내림차순, 등록 sequence 오름차순으로
  결정적으로 전달한다.
- 기본 이동은 `KeyW/A/S/D`와 방향키를 같은
  `moveUp/down/left/right` action에 연결한다.
- 키 문자와 Shift 상태가 아니라 `KeyboardEvent.code`를 사용한다.
- 사용자 오버라이드는 `settings.json`의 `inputBindings` plain object로
  저장하고 runtime 설정 적용 시 즉시 `InputBindingMap`에 반영한다.
- 기본 바인딩은 input 모듈이 소유하며 설정에는 바뀐 action만 저장할 수 있다.
  명시적인 빈 배열은 unbound, action당 유효 code는 최대 4개다.
- runtime 바인딩 교체와 기본값 복원은 raw down/press 상태를 초기화해 이전
  키의 hold/edge가 새 action으로 전이되지 않게 한다.
- 시뮬레이션 공식 필드는 `actionStates`이며 물리 code를 노출하지 않는다.
...
project/game/script/module/input/_input_binding_constants.js
project/game/script/module/input/_input_binding_map.js
project/game/script/module/input/_keyboard_input_handler.js
project/game/script/module/input/input_system.js
...
- `WorldCamera2D`는 실제 표시 `WW × WH`의 전체 맵 contain 배율을 기준으로
  기본 zoom `0.7`을 적용하며 물리·AI 좌표를 변경하지 않는다.
- zoom `0.7`에서는 맵 중심을 표시하고 그보다 확대되면 Tower의 보간된
  `renderPosition`을 viewport 중앙에서 추종한다. 맵 경계에서도 offset을
  clamp하지 않아 월드 밖 배경을 표시한다.
- `TileMapRenderer`는 828개 정적 타일의 viewport 좌표와 크기를 최초 draw와
  projection revision 변경 시에만 다시 계산한다. zoom 애니메이션과 확대
  추종 이동은 revision을 바꾸며, 매 프레임 cache를 WebGL background batch로
  제출한다.
...
- 신규 계약 테스트가 가속, 마찰 정지, force/impulse, 충돌 위치 보정, 보간,
  ...
  HP 부재, `KeyboardEvent.code` 의미 action 매핑을 검증한다.
...
### 3.7 Tower 추종 wheel zoom과 연속 애니메이션

현재 zoom 흐름:

```text
WheelEvent deltaMode 정규화·누적
→ InputSystem/SimulationRuntime wheel totals
→ InputActionMapper가 직전 합계와 차이 계산
→ CAMERA_ZOOM PlayerAction
→ PlayerControlRouter
→ CameraZoomController (IPlayerControllable)
→ The Tower (ICameraFollowTarget2D)
→ WorldCamera2D.zoom
```

- pixel/line/page wheel delta를 무차원 unit으로 정규화하고 event당 절댓값을
  4로 제한한 뒤 누적한다. snapshot은 이 값을 소비하거나 0으로 되돌리지 않는다.
- `GameSystem.enter()`가 현재 합계를 baseline으로 잡아 씬 진입 전 wheel을
  재생하지 않는다. mapper가 마지막 합계와 차이를 갱신하므로 같은 snapshot을
  여러 번 읽어도 action이 중복되지 않는다.
- 카메라 zoom도 `GAMEPLAY` context의 priority 100
  `IPlayerControllable`로 등록된다.
- `WorldCamera2D`는 렌더 전용 `IWorldViewProjection2D`와 제어용
  `ICameraControl2D`를 분리해 구현하고, The Tower는
  `ICameraFollowTarget2D`를 제공한다.
- contain 배율 대비 기본/최소 zoom은 `0.7`, 최대는 `3`, wheel unit당
  `1.16` 비율로 목표값을 누적한다.
- `0.4초 easeOutExpo` 표준 animation을 사용한다. 추가 wheel은 새
  animation을 만들지 않고 같은 handle을 현재 표시값에서 최신 목표로
  retarget한다.
- 목표 또는 현재 zoom이 `0.7`보다 크면 fixed 위치가 아니라 가변 update에서
  계산한 Tower `renderPosition`을 viewport 중앙에 둔다.
- 맵 가장자리에서 projection offset을 제한하지 않는다. Tower는 중앙에
  유지되고 viewport에는 음수 또는 맵 범위 밖 월드 좌표가 보일 수 있다.
- zoom `0.7`로 복귀하면 맵 기하학 중심을 viewport 중앙에 다시 둔다.
  resize는 현재 화면 중심과 zoom을 보존한다.
- zoom은 가변 프레임 presentation이며 물리·AI·저장 좌표를 변경하지 않는다.
- `AnimationSystem` 표준 handle은
  `id/promise/retarget/remove/isActive`를 제공하며 풀 재사용 뒤 stale handle이
  다른 animation을 조작하지 못한다.
- `SliderElement` 표시값도 연속 drag에 같은 handle을 retarget한다.
- 타이틀 tilt/spotlight/border/particle의 개별 delta smoothing은 공통
  `getContinuousInputBlend()`로 이동했고 패널의 기존 함수는 호환 wrapper다.

실제 파일:

```text
project/game/script/module/input/_mouse_input_handler.js
project/game/script/module/ingame/contract/camera_control_contract.js
project/game/script/module/ingame/input/camera_zoom_controller.js
project/game/script/module/ingame/map/world_camera_2d.js
project/game/script/module/ingame/object/the_tower.js
project/game/script/module/ingame/object/game_object_system.js
project/game/script/module/animation/_standard_animation.js
project/game/script/module/animation/animation_system.js
project/game/script/module/animation/_continuous_animation_math.js
project/game/script/module/ui/element/_slider.js
...
- aim, 스킬, pointer button action, edge FIFO
- 키 바인딩 capture/해제/충돌 경고/로컬라이즈 표시를 제공하는 설정 UI
- mouse button/gamepad binding과 키보드 외 장치의 의미 action 매핑
- 카메라 drag pan, 키보드 zoom/reset, touch pinch와 사용자 zoom 감도 설정
...
| snapshot의 `keys`와 `isSimulationKeyboardPressed()` | `main`에서 이어진 기존 소비자 이름을 유지하되 값은 물리 키가 아니라 의미 action임 | 모든 소비자가 `actionStates`와 `isSimulationInputActionPressed()`로 전환 |
...
- `KeyboardEvent.code` raw 상태와 의미 action 바인딩 분리
- 사용자 키 오버라이드 검증, unbound 빈 배열, 기본값 fallback과 깊은 복사
- runtime 바인딩 교체 시 stale raw hold/press edge 초기화
- `inputBindings` plain object 설정의 안전한 복제와 runtime 즉시 적용 경계
- snapshot의 `actionStates`와 임시 `keys` 의미 별칭 동기화
...
- resize에서 월드 좌표·현재 zoom·화면 중심 보존과 카메라 viewport 갱신
- pixel/line/page wheel delta 정규화와 누적 snapshot
- 씬 진입 wheel baseline, 동일 snapshot 중복 방지, 연속 목표 zoom 누적
- `CAMERA_ZOOM`의 PlayerControllable 라우팅과 0.4초 `easeOutExpo`
- `ICameraControl2D`/`ICameraFollowTarget2D` 구조 계약
- 기본/최소 zoom `0.7`, 최대 `3` clamp와 기본 상태 맵 중앙 정렬
...
- 2560×1440 contain 기반 0.7 projection과 1280×720에서 정확한 1/2 표시 배율
- 정적 타일 projection이 같은 revision의 연속 draw에서는 재계산되지 않고
  resize와 zoom의 projection revision 변경 때 갱신
...
- `KeyW/A/S/D`와 방향키의 동일 MOVE action 매핑
...
5. 전용 키 바인딩 설정 UI의 capture, conflict 표시, apply/cancel transaction을
   현재 `inputBindings` runtime port에 연결한다.
6. WorldRegistry와 component/capability index로 현재 Core/Tower/body/collider 등록을 이관한다.
7. 일반 CollisionHandler가 현재 타일 resolver와 object-object pair를 조정하게 한다.
8. 기존 충돌 JS/WASM 커널을 새 CollisionHandler port 뒤에 연결한다.
9. 현재 `west-gate-01`에 첫 적과 waypoint PathFollower를 연결한다.
10. route stage별 Flow Field를 연결한 뒤 두 번째 Gate fixture를 실제 맵 콘텐츠로 확장한다.
11. WaveDirector와 한 웨이브 완료를 연결한다.
12. 웨이브 종료 `ingame.dat` atomic checkpoint를 구현한다.
...
| 2026-07-26 | AnimationSystem 표준 handle retarget/stale-pool 안전 계약, Slider 연속 retarget, title/panel 공통 delta smoothing을 구현 |
