...
const ENEMY_CAPABILITY_CONTRACT_SOURCE = await readFile(
    new URL(
        '../project/game/script/module/ingame/contract/enemy_capability_contract.js',
        import.meta.url
    ),
    'utf8'
);
const ENEMY_SPAWN_ADAPTER_SOURCE = await readFile(
    new URL(
        '../project/game/script/module/ingame/object/enemy/gpu_enemy_spawn_adapter.js',
        import.meta.url
    ),
    'utf8'
);
...
test('invalid modifier/profile numbers는 fail-fast하고 intent는 continuous contact 및 allowlisted metadata만 전달한다', () => {
    ...
    assert.equal(arrowIntent.enemyBehaviorState.chargeSpeedTilesPerSecond, 6);
    assert.equal(arrowIntent.enemyBehaviorState.impactRestitution, 0.55);
    assert.equal(
        arrowIntent.enemyBehaviorState.impactTangentialRetention,
        0.85
    );
