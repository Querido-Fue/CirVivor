...
assert.equal(GPU_CIRCLE_BODY_ABI.ENEMY_BEHAVIOR_STATE.IMPACT_RESTITUTION, 48);
...
assert.equal(
    GPU_CIRCLE_BODY_ABI.ENEMY_BEHAVIOR_STATE.DEPRECATED_CHARGE_ACCELERATION,
    80
);
assert.equal(
    GPU_CIRCLE_BODY_ABI.ENEMY_BEHAVIOR_STATE.IMPACT_TANGENTIAL_RETENTION,
    84
);
assert.equal(GPU_CIRCLE_BODY_ABI.ENEMY_BEHAVIOR_STATE.RECOIL_DAMPING, 88);
assert.equal(
    GPU_CIRCLE_BODY_ABI.ENEMY_BEHAVIOR_STATE.RECOIL_SLEEP_THRESHOLD,
    92
);
...
assert.equal(
    normalizeGpuCircleBodyRenderShapeCode(GPU_CIRCLE_BODY_RENDER_SHAPE.CORK),
    GPU_CIRCLE_BODY_RENDER_SHAPE.CORK
);
...
assert.deepEqual({
    ...
    impactRestitution: 0,
    impactTangentialRetention: 0,
    recoilDamping: 0,
    recoilSleepThresholdTilesPerSecond: 0,
    windupTicks: 0,
    chargeMaxTicks: 0,
    recoilTicks: 0,
    recoverTicks: 0,
...
assertThrowsNamed(() => normalizeGpuCircleBodyRenderShapeCode(12), 'RangeError');
