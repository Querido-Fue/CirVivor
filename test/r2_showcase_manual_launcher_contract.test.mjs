...
async function readGameFile(relativePath) {
    return readFile(relativePath.startsWith('test/')
        ? path.join(TEST_DIRECTORY, relativePath.slice(5))
        : path.join(GAME_DIRECTORY, relativePath), 'utf8');
}
...
test('screenshot은 명시적 action에서만 ignored evidence 경로에 deterministic 이름으로 저장한다', async () => {
    ...
    assert.equal(
        ...
        path.join(TEST_DIRECTORY, 'nw_r2_showcase_manual')
    );
    assert.equal(
