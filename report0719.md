...
### 4.30 enemy-enemy ordered circle resolve의 WASM 이전

- **파일 경로:** `project/game/script/module/physics/_collision_handler.js`, `collision_candidate_pair_buffer.js`, `collision_candidate_pair_processor.js`, `collision_candidate_admission.js`, `collision_enemy_pair_budget.js`, `collision_enemy_sleep_state.js`, `collision_enemy_circle_pair_soa.js`, `collision_pair_resolver.js`, `collision_body_translation.js`, `collision_broadphase_buffer.js`, `_collision_resolve_tuning.js`, `collision_manifold_writer.js`, `project/game/test/collision_enemy_fast_path.test.mjs`, `guide/domain/simulation_native_acceleration_guide.md`.
- **현재 순서 계약:** pass별 base token은 `fixedFrameToken + pairPassCursor`이고 priority range는 이 값, normal range는 여기에 `priorityCount`를 더한 값으로 각각 회전한다. 최대 3개 위치 pass 중 0·2번째 pass가 grid/candidate를 재구축하며 1번째 pass는 직전 목록과 grid를 재사용한다. 한 pair의 해소는 body, Float64 relation, Float32 broad/circle-parts, 원본 `ref.position`과 pass/frame counter를 즉시 갱신하므로 후속 pair의 live narrowphase/resolve 입력이 달라진다. circle 및 circleParts enemy pair는 enemy-pair process budget 대상이고 anchor 여부에 따라 effective budget이 다르다. player/wall object pair는 priority range에 추가되어 enemy priority pair와 회전 순서상 섞이고 `priorityCount`를 통해 normal range의 start token에도 영향을 주지만, 그 non-enemy pair 자체는 enemy-pair budget을 소비하지 않는다.
- **소스 기반 작업량 상한:** 적 800개가 모두 non-anchor circle일 때 low body별 current 12개+predictive 2개 admission을 tail까지 합친 active enemy candidate의 tail-adjusted combinatorial 상한은 `786 × 14 + Σ(1..13) = 11,095` pair/rebuild이다. 세 pass가 모두 실행되면 player/wall guaranteed pair를 제외한 normal-circle enemy-enemy admitted-pair processing loop iteration은 최대 33,285회/tick이다. 실제 narrowphase process-attempt의 정적 상한은 첫 pass `800 × 14 ÷ 2 = 5,600`회이다. 후속 pass는 첫 pass의 resolved count가 양수일 때만 열리고, 이때 dense pressure도 양수여서 `resolveBoost > 1`·stabilize budget 10이 적용되므로 두 pass가 각각 `4,000`회, 합계 13,600회/tick이며 60Hz에서 816,000회/s이다. 이는 실측치가 아니며 AABB/circle reject와 budget skip도 이 processing loop iteration에는 포함된다. 전체 priority/normal loop에는 non-enemy guaranteed pair가 별도로 추가되므로 33,285를 전체 candidate loop 상한으로 해석하지 않는다. anchor pair는 doubled budget을 쓰지만 한쪽이 2-cell 이상 hexa `circleParts`이므로 이 all-circle fast-path 상한과 분리해야 한다.
- **전송량 해석:** 위 11,095개 active low/high `Int32` pair 88,760B, 800 body의 현행 full broad plane 44,800B, relation plane 51,200B, kind/shape plane 1,600B만 합쳐도 process adapter의 one-way numeric snapshot 예시는 186,360B(약 182.0KiB)이다. candidate rebuild용 sweep plane 51,200B와 validity 800B까지 포함한 예시는 238,360B(약 232.8KiB)이다. 두 값은 현행 layout에서 계산한 **복사 시나리오 크기**일 뿐 측정된 전송 비용이나 exact adapter의 hard lower bound가 아니며, mutable body/ref/solver counter, callback 상태, 결과 동기화와 예외 journal은 포함하지 않는다.
- **완전 동일성 차단점:** `Math.sqrt`·`Math.hypot`·`Math.pow`의 raw Float64 결과, pair마다 발생하는 Float32 read-modify-write 반올림, live pressure/weight/frame 이동 상한과 sleep 관측, mixed fallback interleave를 그대로 재현해야 한다. `ref.applyAxisResistance`와 profile/detector 같은 JS 호출은 앞선 필드가 이미 바뀐 뒤 예외를 던질 수 있어 그 pair의 정확한 부분 상태와 same error identity도 보존해야 한다. WASM trap 뒤 JS fallback을 같은 tick에서 재실행하면 이미 적용된 pair를 중복 처리하므로, authoritative memory와 commit journal 없이 안전한 mid-pass 복구도 불가능하다.
- **현재 테스트 공백:** `collision_enemy_fast_path.test.mjs`는 fast/generic 결과를 기본 `1e-8`, broad plane을 `1e-5` 허용 오차로 비교하고 소수 normal case와 budget rotation만 검사한다. raw-bit equality, 3-pass rebuild, 모든 회전 시작점, mixed circleParts/player/wall, Float32 누적 반올림, callback throw의 부분 상태, WASM trap·capacity·memory-grow 복구를 고정하지 않으므로 exact WASM oracle로 사용할 수 없다.
- **현재 처리:** 2026-07-13의 collision 약 89% 수치는 현재 JS SoA fast path 이전의 과거 G0 profile이고, 이후 800명+always-blur가 실제 fixed 60 tick/s·debt 0 gate를 통과했다. 현 상태에서 pair별 export나 JS-owned circle-only batch를 구현할 근거가 없어 생산 코드·테스트·WASM artifact를 변경하지 않는 **NO-GO**로 판정했다.
- **재검토 조건:** fresh release G0에서 후보가 frame CPU 25% 이상 또는 p95 4.17ms 이상임을 다시 입증한 뒤, WASM이 canonical SoA를 직접 소유하는 `grid → candidate → 전체 narrowphase → ordered resolve` coarse authority만 검토한다. 별도 legacy JS oracle로 raw IEEE-754/부호 있는 0/비유한 값, 접선 양쪽 1-ULP, 3-pass·rebuild·rotation·budget·sleep·mixed fallback, 모든 callback/throw/trap 부분 상태를 pair마다 exact 비교하고 10,000 tick의 전체 state·event·command를 원시 바이트로 일치시켜야 한다. 이어 실제 NW.js에서 pack/copy/commit을 포함한 G2와 kernel p95 1.3배·전체 fixed p95 15% 개선·400명 회귀 5% 이하의 G5를 모두 통과할 때만 승격한다.
...
### 5.10 비네트 ordered-dither offset 사전 계산

- **파일 경로:** `project/game/script/module/display/_vignette_renderer.js`, `project/game/script/data/display/vignette_constants.js`, `project/game/script/util/number_util.js`.
- **문제 후보와 실행 빈도:** `_colorizeBlur()`는 비네트 캐시가 dirty인 resize·render-scale·theme 변경 시 blur mask의 alpha가 0보다 큰 픽셀마다 `_resolveAlphaByte()`를 호출한다. 현재 4×4 Bayer 정수 셀을 `/ 16`한 뒤 `0.46875`를 빼므로, 16개의 offset을 미리 저장하면 픽셀별 나눗셈과 뺄셈을 제거할 수 있어 감사했다. 이 경로는 steady frame마다 실행되는 렌더 루프가 아니라 캐시 재구축 hitch 경로이다.
- **정상 숫자 환경의 수학 동등성:** 정수 셀 `m`에 대한 기존 offset은 `(m / 16) - 0.46875 = (2m - 15) / 32`이다. 16개 값과 두 중간 연산은 모두 binary64에서 정확히 표현되는 dyadic이므로, native intrinsic과 초기화 뒤 불변 행렬을 전제로 같은 중첩 인덱스 순서를 유지한 2D offset table은 임의 Number strength에 대해 `Math.round()` 입력과 최종 alpha byte를 비트 단위로 보존할 수 있다.
- **결정적 동일성 반례:** 모듈 초기화 전 교체 가능한 `Object.freeze`는 기존 4개 행과 외부 배열을 총 5회 관찰하고 그 참조를 캡처할 수 있다. freeze를 no-op으로 두고 캡처한 첫 셀을 로드 뒤 `8`로 바꾸면 기존 구현은 `(8 / 16) - 0.46875 = 0.03125`를 사용하지만 offset table 후보는 같은 값을 offset `8`로 해석한다. alpha `93`, strength `1.1`, phase `(0, 0)`의 실제 byte는 변이 전 legacy/table 모두 `92`지만 변이 뒤 `93`/`102`로 갈라진다. 후보는 처음부터 다른 음수·분수 배열을 freeze에 전달하므로 초기화 인수 자체도 관찰 가능하다. getter·Proxy·`Symbol.toPrimitive`가 cell에 개입하면 `/`와 `*`의 coercion 지점, 예외와 재진입 순서도 달라질 수 있다.
- **호환 fallback의 성능 실패:** 원래 정수 행렬과 `[y & 3][x & 3]` 조회를 유지하고 primitive 0~15만 `switch`의 exact offset으로 바꾸며 나머지는 기존 수식으로 fallback하는 후보는 위 cell mutation·coercion 의미를 대부분 보존한다. 그러나 Node 22.19.0/V8 12.4.254.21의 실제 production source 기반 1920×1080, warmup 7쌍, AB/BA 31쌍 측정에서 resolver p50/p95는 37.7190/38.4434→45.1869/46.6380ms(0.8347/0.8243배), alpha 점유 39.9853%의 `_colorizeBlur` loop는 23.8988/24.0510→26.6038/26.9175ms(0.8983/0.8935배), 100% 점유는 52.4191/53.5227→62.7242/63.5373ms(0.8357/0.8424배)로 명확히 느려졌다. 복사 포함 p50도 각각 0.8753배·0.8405배였다.
- **단순 table의 제한적 성능:** 같은 측정에서 직접 2D offset table은 resolver p50/p95 37.7387/39.1788→38.8828/40.0433ms(0.9706/0.9784배)로 느렸지만, 39.9853% 점유 colorize loop는 23.8392/24.0962→23.1202/23.3495ms(1.0311/1.0320배), 100% 점유는 52.7549/53.0236→50.4906/50.7317ms(1.0448/1.0452배)였다. 복사 포함 p50/p95 향상은 각각 1.0319/1.0284배와 1.0299/1.0393배였다. 각 variant의 측정 전후 8,294,400 RGBA byte는 native 정상 환경에서 exact 일치했다. 최대 약 4.5%의 dirty colorize 구간 이득은 모듈 초기화·변이 반례를 감수할 근거가 아니며, blur·putImageData·최종 합성을 포함한 전체 캐시 재구축 향상도 입증하지 못한다.
- **현재 처리:** 모든 edge에서 완전 동일하다는 요구를 충족하지 못한 단순 table과 성능이 크게 악화된 호환 fallback을 모두 미채택했다. production 렌더 소스 SHA-256 `4109cd5e6a039d42ee7241587342926187949b470662962396853e3efeeae39a`는 감사 전과 동일하며 테스트·WASM artifact도 추가하지 않았다.
- **재검토 조건:** `Object.freeze`와 숫자 intrinsic이 native이고 초기 행렬을 외부에서 관찰·변경하지 않는다는 지원 계약을 명시적으로 승인한 경우에만 actual-source 양방향 테스트를 만든다. 16 phase, alpha 0~255와 half-step 양쪽 1-ULP, ±0·subnormal·`NaN`·±`Infinity`, x/y/cell coercion·예외·재진입, `_colorizeBlur` 전체 RGBA·부분 쓰기·override dispatch를 exact 비교하고, 실제 NW.js의 blur·colorize·putImageData·최종 compositor RGBA 0-byte 차이와 dirty-cache p50 1.01배 이상·p95 비퇴행을 모두 통과해야 한다.
